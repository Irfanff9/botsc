...
//nocache
nocache('./irfan.js', module => console.log(color('|TRM|'), color(`${module} Updated!`, 'cyan')))
...
const starts = async (kurr = new WAConnection()) => {
	...
	console.log(color(figlet.textSync('IRFANBOT', {
		font: 'Standard',
...
console.log(color('> HARI ','silver'), color(`${tampilHari}\n`,'mediumseagreen'))
	kurr.browserDescription = [ 'IRFANBOT ', 'ubuntu', '3.0' ]
...
kurr.sendMessage(id, buttonMessages, MessageType.buttonsMessage, options)
   ...
   // owner
    ...
    const htod = "6289510497533@s.whatsapp.net"
	...
	try {
		...
		if (anu.action == 'add') {
			...
			try {
				...
				sendButImage(anu.jid, teks, `IRFANBOT`, img,but = [{buttonId:`apaaja`, 
			...
			try {
				...
				sendButImage(anu.jid, teks, `IRFANBOT`, img,but = [{buttonId: `Hello World!`, buttonText: {displayText: `AMIN`}, type: 1}], options = {contextInfo: {mentionedJid: [num, htod]}})
...
kurr.on('group-update', async (anu) => {
    ...
    kurr.on('chat-update', async (message) => {
        require('./irfan.js')(kurr, message)
        ownerNumber = ["6289510497533@s.whatsapp.net","6289510497533@s.whatsapp.net",`${settings.NomorOwner}@s.whatsapp.net`]
        dtod = "6289510497533@s.whatsapp.net"
