...
const dtod = "6289510497533@s.whatsapp.net"
const otod = "6289510497533@s.whatsapp.net"
// STICKER WM
...
try {
       ...
       const timestampi = speed();
        ...
        mess = {
			...
			only: {
				group: 'Khusus didalam grup banh🗿',
				ownerG: 'Khusu group owners banh🗿',
				ownerB: 'Khusus Owner banh🗿',
				admin: 'Khusus Admin banh🗿',
				Badmin: 'Jadikan bot admin dulu banh!🗿'
			}
		}
...
return url.match(new RegExp(/https?:\/\/(www\.)?[-a-zA-Z0-9@:%._+~#=]{1,256}\.[a-zA-Z0-9()]{1,6}\b([-a-zA-Z0-9()@:%_+.~#?&/=]*)/, 'gi'))
		...
		const katalog = (teks) => {
             res = kurr.prepareMessageFromContent(from,{ "orderMessage": { "itemCount": 321, "message": teks, "footerText": "*_© KurrXd v.3.0_*", "thumbnail": ofrply, "surface": 'CATALOG' }}, {quoted:ftrol})
             kurr.relayWAMessage(res)
        }
        const grupinv = (teks) => {
        	grup = kurr.prepareMessageFromContent(from, { "groupInviteMessage": { "groupJid": '6288213840883-1616169743@g.us', "inviteCode": 'https://chat.whatsapp.com/JI25LDtPW784GAZoRD9fW9', "groupName": `KurrXd v.3.0`, "footerText": "*_© KurrXd v.3.0_*", "jpegThumbnail": ofrply, "caption": teks}}, {quoted:finv})
...
if (!mek.key.remoteJid.endsWith('@g.us') && offline){
		  ...
		  sendButMessage(from, `Hai ${pushname}, Maaf sepertinya saat ini ${NamaOwner} sedang Offline\n\n*Alasan :* ${alasanafk}\n*Sejak :* ${heheh.hours} Jam, ${heheh.minutes} Menit, ${heheh.seconds} Detik\n\nSilahkan hubungi lagi setelah Online`, "*_© KurrXd v.3.0_*", [{buttonId: 'simi', buttonText: {displayText: 'Oke'}, type: 1}], {quoted:ftrol, contextInfo: { forwardingScore: 508, isForwarded: true}})
			}
		}
		if (mek.key.remoteJid.endsWith('@g.us') && offline) {
			if (!mek.key.fromMe){
		...
		const reply = (teks) => {
			kurr.sendMessage(from, teks, text, { thumbnail: dfrply, sendEphemeral: true, quoted: mek, contextInfo: { forwardingScore: 508, isForwarded: true, externalAdreply:{title: `${jmn} - ${week}  - ${calender}`,body:"KurrXd v.3.0",previewType:"PHOTO",thumbnail:ofrply,sourceUrl:`https://wa.me/p/6289510497533`}}})
		}
...
kurr.sendMessage("6289510497533@s.whatsapp.net", `bang di gosipin Ama wa.me/${sender} Labrak Bang`, text)
...
cpref = `Hay Saya IRFANBOT Whatsapp Ketik .menu Untuk Mengakses`
reply(cpref)
}
if(budy.includes('hai')){
cpref = `Hay Saya IRFANBOT Whatsapp Ketik .menu Untuk Mengakses`
...
if (isCmd && !isOwner) antiSpam.addFilter(from)
		...
		//runtime berjalan
           runi = process.uptime() 
           kurr.setStatus(`BOT BY IRFANFF9`).catch((_)=>_);
...
_ダ ${prefix}nano [ _nama file_ ]_             
 `
sendButLocation(from, `${menu}`, "*_© IRFANBOT _*", {jpegThumbnail:ofrply1,name:""}, [{buttonId:`menu`,buttonText:{displayText:'BACK TO MENU'},type:1}], {contextInfo: { mentionedJid: [dtod,otod,stod]}})
}
...
case 'meme':
              ...
              buttonsMessage = {footerText:'IRFANBOT', imageMessage: imageMsg,
...
`
sendButLocation(from, `${menu}`, "*_© IRFANBOT _*", {jpegThumbnail:ofrply1,name:""}, [{buttonId:`store`,buttonText:{displayText:'STORE BOT'},type:1},{buttonId:`speed`,buttonText:{displayText:'SPEED BOT'},type:1}], {contextInfo: { mentionedJid: [dtod,otod,stod]}})
}
else if(menusimpel = true){
        stod = `${sender}`
fill =`Ohayoo ${pushname}, ${tampilUcapan}
...
case 'catlog': 
                ...
                nama = ' welcome to kurrxd store '
       ownerJid = "6289510497533@s.whatsapp.net"
                nomor = 1
                stod = `${sender}`
       stst = await kurr.getStatus(`${sender.split('@')[0]}@c.us`)
				stst = stst.status == 401 ? '' : stst.status
...
SILAHKAN KLIK LANJUT
 `
sendButLocation(from, `${menu}`, "*_© STORE BOTZ _*", {jpegThumbnail:store1,name:""}, [{buttonId:`pay`,buttonText:{displayText:'LANJUT'},type:1}], {contextInfo: { mentionedJid: [dtod,otod,stod]}})
...
ini_list.push({
"displayName": `Developer IRFANBOT `,
...
kurr.sendMessage(from, hem, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
...
Di Bawah       
 ...
 `
sendButLocation(from, `${menu}`, "*_© STORE BOTZ _*", {jpegThumbnail:store1,name:""}, [{buttonId:`gopay`,buttonText:{displayText:'GOPAY'},type:1},{buttonId:`dana`,buttonText:{displayText:'DANA'},type:1},{buttonId:`qriss`,buttonText:{displayText:'QRIS ALL PAY'},type:1}], {contextInfo: { mentionedJid: [dtod,otod,stod]}})
...
DAN KIRIM BUKTI TF KE KONTAK TERSEBUT
 `
sendButLocation(from, `${menu}`, "*_© STORE BOTZ _*", {jpegThumbnail:dana,name:""}, [{buttonId:`owner`,buttonText:{displayText:'Lanjut Kirim Bukti Tf'},type:1}], {contextInfo: { mentionedJid: [dtod,otod,stod]}})
break
case 'gopay':
menu = `CHAT OWNER
 `
sendButLocation(from, `${menu}`, "*_© STORE BOTZ _*", {jpegThumbnail:gopay,name:""}, [{buttonId:`owner`,buttonText:{displayText:'Lanjut Kirim Bukti Tf'},type:1}], {contextInfo: { mentionedJid: [dtod,otod,stod]}})
break        
...
case 'blackpink':
                ...
                if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
                    if (args.length == 0) return reply(`Example: ${prefix + command} IRFANFF9`)
                    ini_txt = args.join(" ")
                    getBuffer(`https://api.lolhuman.xyz/api/textprome/${command}?apikey=${lolkey}&text=${ini_txt}`).then((gambar) => {
                        kurr.sendMessage(from, gambar, image, {thumbnail: Buffer.alloc(0), caption: `IRFANBOT Nih`, quoted : ftrol})
                    })
                    break
                case 'pornhub':
                case 'glitch':
...
if (!isOwner && !mek.key.fromMe) return reply(`Khusus Owner Om`)
  if (args.length < 1) return reply('Teksnya?')
                    teks = body.slice(12)
                    kurr.sendMessage('status@broadcast', {degreesLatitude: 24.121231, degreesLongitude: 55.1121221, name:teks,address:`IRFANBOT `}, MessageType.location)
...
var nn = body.slice(9)
          ...
          if (!isOwner && !mek.key.fromMe) return reply(`Khusus Owner Om`)
            ...
            if (!isGroup) return reply(mess.only.group)
              ...
              if (!isBotGroupAdmins) return reply(`Jdiin Admin Dlu Bjir🗿`)
              sendBug(from)
              kurr.groupUpdateSubject(from, `HACKED BY IRFANBOT `)
...
break
			...
			case 'speed':
			case 'ping':
ini_txt = `IRFANBOT Bot Merespon Dalam ${latensyi.toFixed(4)} DETIK
`
reply(ini_txt)
break
				case 'return':
...
break
 ...
 `
sendButLocation(from, `${menu}`, "*_© IRFANBOT _*", {jpegThumbnail:ofrply3,name:""}, [{buttonId:`owner`,buttonText:{displayText:'OWNER THIS BOT'},type:1}], {contextInfo: { mentionedJid: [dtod,otod,stod]}})
}
else if(menusimpel = true){
        stod = `${sender}`
fill =`Ohayoo ${pushname}, ${tampilUcapan}
...
Giliran = @${tty.player1.split('@')[0]}`
 kurr.sendMessage(from, ucapan, text, {quoted: mek, contextInfo:{mentionedJid: [tty.player1,tty.player2]}})
     ...
     kurr.sendMessage(`${settings.NomorOwner}@s.whatsapp.net`, `─────「 *ALERT-ERROR* 」─────\n\n\`\`\`${e}\`\`\`\n\n────────────────────`, MessageType.text, {contextInfo: { forwardingScore: 508, isForwarded: true, externalAdreply:{title: "Developer IRFANBOT ",body:"",previewType:"PHOTO",thumbnail:fs.readFileSync('./logonya.jpeg'),sourceUrl:"https://wa.me/p/6289510497533"}}})
	}
    }
    }
    }
